package com.macroyau.blue2serial.blue2serial;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MESSAGE_WRITE:
                    ...
                    break;
                case MESSAGE_READ:
                    Log.d(TAG, "handleMessage: PRINT");
                    byte[] bufferRead = (byte[]) msg.obj;
                    //int b = bufferRead[0];
                    //String a = String.valueOf(bufferRead[0]);
                    //Log.d("TESTO", Integer.toString(b));
                    //System.out.printf("0x%02X", bufferRead[0]);

                    String messageRead = new String(bufferRead);
                    //Log.d("TEST", Arrays.toString(bufferRead));
                   /* for (int i = 0; i < bufferRead.length ; i++) {
                       // Log.d("FEST " + i,Byte.toString(bufferRead[0]));

                        System.out.printf("%c",bufferRead[0]);
                    }
                    ...
                    mListener.onBluetoothSerialRead(sb.toString());
                    if (isRaw) {
                        Log.d(TAG, "handleMessage: RAW");
